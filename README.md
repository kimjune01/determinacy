...
**What you bring:** a SOTA coding agent on the CLI (`codex exec -`, `claude -p`, …), plus `git` and `ripgrep`. The tool shells out to your agent for the judgment-shaped steps (proposing candidates and refuting them); swapping agents is a one-line config change. A model never *sets* a verdict, though: it only proposes what a grep, a grader, or a pointer you check yourself then settles.

**The pipeline.** Determinacy auditing is not one screen. It is a funnel that drives candidates through progressively harder certifications, claiming only what survives. The full method (developed over the [reference audit](https://github.com/kimjune01/swebench-pro-audit) of 728 tasks) has these phases. `[tool]` marks what `determinacy run` automates today; the rest is the reference pipeline, some of it requiring the benchmark's own execution harness.
